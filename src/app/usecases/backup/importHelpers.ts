...
export function getImportCounts(backup: BackupData): ImportCounts {
    ...
    return {
        ...
        // Counted on the RAW backup, before `normalizeModernBackup` runs, and
        // with `isValidISODate` rather than a blank test: `normalizeDate` maps
        // both a missing date and a malformed one (e.g. "15.03.2024") to "",
        // so counting after normalization would be correct but counting only
        // blanks before it would miss every malformed row that is about to
        // become one.
        undatedBookings: bookings
            .filter((entry) => !isValidISODate(String(entry.cBookDate ?? "")))
            .length
    };
}
