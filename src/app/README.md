...
- `src/adapters/driven/appAdapter.ts` (bootstrap/orchestration adapter used by entrypoints)
...
database adapter layer in `../adapters/driven/database`:

- `connectionManager.ts` opens/closes the raw `IDBDatabase` connection.
- `migrator.ts` (`setupDatabase`) creates object stores and indexes on version upgrade (accounts, bookings,
  bookingTypes, stocks — see `INDEXED_DB.STORE.*`).
...
`../adapters/driven/database/repositories` (`accountRepository.ts`,
...
`../adapters/ui/stores` (`accounts.ts`, `bookings.ts`, `bookingTypes.ts`, `stocks.ts`,
...
- Hydration: `recordsHub.init(...)` → `initRecordsUsecase` (`usecases/records/init.ts`)
  → `initializeRecords` (domain logic) fills the leaf stores from a `RecordsDbData`
  snapshot that was read out of IndexedDB via the repositories/database adapter (`getAccountRecords`). This happens on
  app boot and on every account switch/delete.
...
the app enforces a single app tab).
