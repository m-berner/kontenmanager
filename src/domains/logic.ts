...
export class DomainLogic {
     ...
     */
    static aggregateBookingsPerType(bookings: BookingDb[], bookingTypes: {
        cID: number,
        cName: string
    }[], year?: number): NumberStringPair[] {
     ...
     */
    static calculatePortfolioByStockId(bookings: BookingDb[], stockId: number): number {
        return bookings.reduce((acc, entry) => {
     ...
     */
    static calculateInvestByStockId(bookings: BookingDb[], stockId: number): number {
    ...
    static async initializeRecords(
        ...
        // Sort bookings
        stores.bookings.items.sort((a: BookingDb, b: BookingDb) => new Date(b.cBookDate).getTime() - new Date(a.cBookDate).getTime())
        ...
        const session = await storage.getStorage([SESSION_STORAGE.HIDE_IMPORT_ALERT.key])
