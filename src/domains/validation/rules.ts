...
export class ValidationRules {
    ...
    static validateIBAN(iban: string): DomainValidationResult {
        ...
        if (cleaned.length !== IBAN_LENGTH_CODES[countryCode as keyof typeof IBAN_LENGTH_CODES]) {
    ...
    static validateISIN(isin: string): DomainValidationResult {
        ...
        if (!cleaned) return {isValid: false, error: VALIDATION_CODES.REQUIRED}
        if (cleaned.length !== 12) return {isValid: false, error: VALIDATION_CODES.INVALID_LENGTH}
        ...
        const checkDigit = this.calculateLuhnCheckDigit(numericString, RULE_CODES.LUHN_BASE)
    ...
    static validateSWIFT(swift: string): DomainValidationResult {
        ...
        if (!/^[A-Z]{4}$/.test(bankCode)) return {isValid: false, error: VALIDATION_CODES.INVALID_BANK}
        ...
        if (!/^[A-Z0-9]{2}$/.test(locationCode)) return {isValid: false, error: VALIDATION_CODES.INVALID_REGION}

        if (cleaned.length === 11) {
            const branchCode = cleaned.substring(8, 11)
            if (!/^[A-Z0-9]{3}$/.test(branchCode)) return {isValid: false, error: VALIDATION_CODES.INVALID_BRANCH}
        }
    ...
    private static calculateLuhnCheckDigit(numericString: string, base: number): number {
        let sum = 0
