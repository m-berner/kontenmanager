...
Domain utilities around `browser.storage.*` that are reused by services/stores.
...
- `codes.ts`: Stable, language-agnostic validation codes (see also `src/constants.ts`).
