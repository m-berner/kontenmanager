...
export class ImportExportValidator {
    ...
    private static checkUndefinedIds(backup: BackupData): string[] {
        ...
        if (undefinedAccounts > 0) errors.push(`${undefinedAccounts} accounts have undefined IDs`)
        const undefinedStocks = backup.stocks.filter(s => s.cID === undefined).length
        if (undefinedStocks > 0) errors.push(`${undefinedStocks} stocks have undefined IDs`)
        const undefinedBookings = backup.bookings.filter(b => b.cID === undefined).length
        if (undefinedBookings > 0) errors.push(`${undefinedBookings} bookings have undefined IDs`)
        return errors
    }
    ...
    private static checkDuplicateIds(backup: BackupData): string[] {
        ...
        if (duplicateAccounts.length > 0) errors.push(`Duplicate account IDs: ${duplicateAccounts.join(', ')}`)
        const duplicateStocks = this.findDuplicates(backup.stocks.map(s => s.cID).filter((id): id is number => id !== undefined))
        if (duplicateStocks.length > 0) errors.push(`Duplicate stock IDs: ${duplicateStocks.join(', ')}`)
        const duplicateBookings = this.findDuplicates(backup.bookings.map(b => b.cID).filter((id): id is number => id !== undefined))
        if (duplicateBookings.length > 0) errors.push(`Duplicate booking IDs: ${duplicateBookings.join(', ')}`)
        return errors
    }
    ...
    private static validateBusinessRules(backup: BackupData): string[] {
        const errors: string[] = []
        for (const booking of backup.bookings) {
            if (booking.cCredit < 0 && booking.cDebit < 0) errors.push(`Booking ${booking.cID} has negative credit/debit values`)
            if (booking.cCredit > 0 && booking.cDebit > 0) errors.push(`Booking ${booking.cID} has positive credit/debit values`)
        }
        return errors
    }
