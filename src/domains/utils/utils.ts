 ...
 *
 * @param {string} isoDate - An ISO 8601 formatted date string.
 * @return {number} The number of milliseconds since the Unix epoch corresponding to the given date.
 */
...
export function log(msg: string, data?: unknown, level?: LogLevelType): void {
    ...
    const logFn = level === undefined ? console.log : (console as any)[level];
 ...
 */
export function memorize<T extends (..._args: any[]) => any>(fn: T): T {
