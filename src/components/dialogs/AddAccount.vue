...
const addBookingTypesForAccount = async (
  accountId: number
): Promise<boolean> => {
  ...
  const bookingTypes: Omit<BookingTypeDb, "cID">[] = [
    ...
    {
      cName: UtilsService.normalizeBookingTypeName(
        t("components.dialog.addAccount.bookingTypes.sell")
      ),
      cAccountNumberID: accountId
    },
    {
      cName: UtilsService.normalizeBookingTypeName(
        t("components.dialog.addAccount.bookingTypes.dividend")
      ),
      cAccountNumberID: accountId
    }
  ];
  ...
  try {
    for (const bookingType of bookingTypes) {
      ...
      const addBookingTypeID = await addBookingType(
        bookingType as Omit<BookingTypeDb, "cID">
      );

      if (addBookingTypeID === -1) {
        UtilsService.log(
          "ADD_ACCOUNT: Failed to add booking type",
          bookingType
        );
...
const onClickOk = async (): Promise<void> => {
  ...
  await submitGuard({
    ...
    operation: async () => {
      ...
      if (addAccountID === -1) {
        UtilsService.log(
          "ADD_ACCOUNT: onClickOk",
          t("components.dialogs.addAccount.messages.error")
        );
