...
- Creating a depot account seeds Buy/Sell/Dividend types with explicit roles (`createDefaultBookingTypes` in
  `usecases/accounts.ts`). Flipping an existing account's `withDepot` on seeds
...
Six financial data providers are supported, plus a `none` entry that disables online price fetching (`FETCH.PROVIDERS`,
keyed `none` / `wstreet` / `fnet` / `acheck` / `goyax` / `ard` / `tgate`):
...
// Both of these need more than invalidation — see §12.1. They share one
// refreshRates() (and its abort controller): whichever fires last wins.
...
watch(() => settings.service, async () => {
    ...
    if (isInitialized.value) await refreshRates()
    else runtime.clearStocksPages()
})

// refreshRates(): await appAdapter.refreshExchangeRates(...), then
...
- `clearStocksPages()` runs **after** the await. Before it, the re-fetch would convert with the divisors being replaced;
  it is also what supersedes in-flight loads, since it clears every page's generation entry (generations come from one
  store-wide counter that is never reset, so an in-flight load's number can match neither the missing entry nor any
  later re-bump, and its write-back is discarded). It only **invalidates freshness markers**, though — see the
  correction below for what actually repaints an already-mounted `CompanyContent`.
...
- The `settings.service` watcher shares this same `refreshRates()` (and its abort controller) rather than only
  invalidating caches: `"none"` is a selectable provider, and both FX writers skip entirely while it's active, so
  switching to a real provider needs the same re-fetch the currency watcher gets, not just a cache bust.

**`clearStocksPages()` does not itself make anything repaint.** It clears freshness markers a *future*
`CompanyContent.onBeforeMount` (or a future `onCurrentItems` page/sort change) will consult — it does not touch
whatever is already rendered. A quote that finished loading (with the wrong divisor: either the pre-fetch `1` or the
previous currency's now-stale rate) inside the window between the reset and the new rates landing stays on screen,
marked fresh, until something else triggers a fetch. If the user simply stays on the Company view — no pagination, no
re-sort — nothing else does. `CompanyContent.vue` closes that gap with its own `watch(() => [runtime.curUsd,
runtime.curEur], ...)`, gated on having already loaded once, that re-fetches exactly the currently rendered rows
whenever the divisors actually change while the view is mounted. `updateAccountUsecase`'s own `clearStocksPages()` call
for a `cCurrency` edit relies on the same watcher for a live repaint; without it, invalidating only prepares a correct
re-fetch for whenever the view next mounts.
...
are persisted exactly as entered, and `cCurrency` records which currency that was. Converting on write would destroy the
transaction's real
