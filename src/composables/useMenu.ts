...
import {hasBookings} from "@/domains/logic";
...
export function useMenuAction(translate?: (_key: string) => string) {
     ...
     */
    const resolveMessage = (key: Parameters<typeof browserService.getMessage>[0] | string): string => {
        if (!translate) {
            return "System error: resolveMessage";
        }
        const translated = translate(key);
        return translated && translated !== key ? translated : browserService.getMessage(key as Parameters<typeof browserService.getMessage>[0]);
    };
