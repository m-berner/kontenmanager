...
export function useValidation() {
    ...
    function createRule(validator: (_value: unknown) => boolean, message: string): T_Validation_Rule {
    ...
    function convertToNumericString(text: string): string {
        return Array.from(text).map(char => {
            ...
            return char  // numbers stay unchanged
        }).join('')
    }

    // Basic rules
    const rules = {
        required: (message: string): T_Validation_Rule => {
            return createRule(
        ...
        stringLength: (min: number, max: number, message: string): T_Validation_Rule => {
            return createRule(
                (v) => {
        ...
        regex: (pattern: RegExp, message: string): T_Validation_Rule => {
            return createRule(
                (v) => {
                    const cleaned = cleanString(v)
                    if (!cleaned) return false
        ...
        oneOfTwo: (zeroValue: Ref<number> | number, message: string): T_Validation_Rule => {
            return createRule(
                (v) => {
     ...
     * @param msgArray error messages [required, length, format, checksum]
     */
    function ibanRules(msgArray: string[]): T_Validation_Rule[] {
        const ibanLength = createRule(
            v => {
                const cleaned = cleanString(v)
                if (!cleaned) return false
    ...
    function calculateLuhnCheckDigit(numericString: string, base: number): number {
        let sum = 0
        let alternate = true
    ...
    /**
     * ISIN validation with Luhn-Algorithm
     * @param msgArray error messages [required, length, format, country, checksum]
     */
    function isinRules(msgArray: string[]): T_Validation_Rule[] {
        const countryCode = createRule(
            v => {
                const cleaned = cleanString(v)
                if (!cleaned) return false
    ...
    function swiftRules(msgArray: string[]): T_Validation_Rule[] {
        ...
        const branchCode = createRule(
            (v) => {
                const cleaned = cleanString(v)
                if (!cleaned || cleaned.length !== 11) return true
    ...
    return {
        ...
        // Simple rule sets
        nameRules: (msgArray: string[]) => [
            rules.required(msgArray[0]),
            rules.stringLength(2, 32, msgArray[1]),
            rules.regex(/^\p{L}/u, msgArray[2])
        ],

        amountRules: (zeroValue: Ref<number> | number, msgArray: string[]) => [
            rules.oneOfTwo(zeroValue, msgArray[0])
        ],

        bookingTypeRules: (msgArray: string[]) => [
            rules.required(msgArray[0])
        ]
    }
}
