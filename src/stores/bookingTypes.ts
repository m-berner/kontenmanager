...
export const useBookingTypesStore = defineStore('bookingTypes', function () {
    ...
    const getNameById = computed(() => (ident: number): string => {
        const bookingType = items.value.find((entry: BookingTypeDb) => entry.cID === ident)
        return bookingType !== undefined ? bookingType.cName : ''
    })

    /** Resolves a bookingType by its ID. */
    const getItemById = computed(() => (id: number): BookingTypeDb => items.value[getIndexById.value(id)])
    ...
    const isDuplicate = computed(() => (name: string, excludeId?: number): boolean => {
        const normalizedInput = UtilsService.normalizeBookingTypeName(name)
        return items.value.some((entry: BookingTypeDb) => {
            const isSameName = UtilsService.normalizeBookingTypeName(entry.cName) === normalizedInput
