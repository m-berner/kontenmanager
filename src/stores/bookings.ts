...
export const useBookingsStore = defineStore('bookings', function () {
    ...
    const getItemById = computed(() => (id: number): BookingDb => items.value[getIndexById.value(id)])
    ...
    const hasBookingType = computed(() => (ident: number): boolean => {
        const findings = items.value.filter((entry: BookingDb) => entry.cBookingTypeID === ident)
        return findings.length > 0
    })

    /** Checks if any bookings are associated with a specific stock. */
    const hasStockID = computed(() => (ident: number): boolean => {
        const findings = items.value.filter((entry: BookingDb) => entry.cStockID === ident)
        return findings.length > 0
    })
    ...
    const dividendsByStockId = computed(() => (ident: number) => {
        return items.value.filter((entry: BookingDb) => {
    ...
    const bookedYears = computed(() => {
        const years = items.value.map((entry: BookingDb) => (new Date(entry.cBookDate).getFullYear()))
