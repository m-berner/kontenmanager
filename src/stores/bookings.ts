...
import {calculateSumAllFees, calculateSumAllTaxes, calculateSumFees, calculateSumTaxes, aggregateBookingsPerType, calculatePortfolioByStockId, calculateInvestByStockId, calculateTotalSum} from "@/domains/logic";
...
export const useBookingsStore = defineStore("bookings", function () {
    ...
    const sumBookingsPerTypeAndYear = computed(() => (y: number) => {
        ...
        return aggregateBookingsPerType(items.value, bt.items, y);
    });

    /** Total aggregated booking sums per type. */
    const sumBookingsPerType = computed(() => {
        const bt = useBookingTypesStore();
        return aggregateBookingsPerType(items.value, bt.items);
    });
    ...
    const portfolioByStockId = computed(() => (ident: number) => {
        return calculatePortfolioByStockId(items.value, ident);
    });
    ...
    const investByStockId = computed(() => (ident: number) => {
        return calculateInvestByStockId(items.value, ident);
    });
    ...
    function add(booking: BookingDb, prepend: boolean = false): void {
        log("STORES bookings: add", booking, "info");
