...
Type note: the project keeps `@/domain/types` as a stable import path, but the underlying type definitions are split
into layer-focused modules under `src/domain/types/` (domain/adapter/backup/ui) and re-exported by
...
- `logic.ts`: aggregateBookingsPerType, calculateInvestByStockId, calculatePortfolioByStockId, calculateSumAllFees,
