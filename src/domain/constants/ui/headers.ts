...
export const createDividendHeaders = (t: (_key: string) => string): readonly HeaderItem[] =>
    [
        createHeaderItem(t, TRANSLATION_KEYS.DIVIDEND_YEAR, "exDate"),
...
export const createHomeHeaders = (t: (_key: string) => string): readonly HeaderItem[] =>
    [
        ...
        // `cBookingTypeID` is the real field; the old "cBookingType" matched no
        // property on a BookingDb at all. Inert while the column is unsortable
        // and HomeContent renders every cell through its own `item` slot, but a
        // phantom key silently no-ops the moment someone enables sorting.
        //
        // Must STAY unsortable: the cell displays a resolved name
        // (bookingTypes.getNameById), so sorting by this key would order rows by
        // the type's auto-increment id — i.e. by creation order, not
        // alphabetically — which is not what the visible column implies.
        createHeaderItem(t, TRANSLATION_KEYS.HOME_BOOKING_TYPE, "cBookingTypeID")
    ] as const;
