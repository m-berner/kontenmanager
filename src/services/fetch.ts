...
function parseCurrency(code: string): string {
    ...
    if (code.includes("EUR") || code.includes("€")) {
        return "EUR";
    }
...
function extractGoyaxRate(doc: Document): string {
    ...
    try {
        ...
        if (nodes.length < 2) {
            log("SERVICES fetch", {parser: "extractGoyaxRate", reason: "missing overview nodes", selector: OVERVIEW_SELECTOR, nodes: nodes.length}, "warn");
            return DEFAULT_VALUE;
        }
        ...
        if (listRows.length < 2) {
            log("SERVICES fetch", {parser: "extractGoyaxRate", reason: "missing list-rows", listRows: listRows.length}, "warn");
            return DEFAULT_VALUE;
        }
        ...
        if (listItems.length < 4) {
            log("SERVICES fetch", {parser: "extractGoyaxRate", reason: "missing list items", listItems: listItems.length}, "warn");
            return DEFAULT_VALUE;
        }

        const rateText = listItems[3].textContent ?? DEFAULT_VALUE;

        // Parse format: "...(xxx)rate"
        const parts = rateText.split(")");

        return parts.length > 1
            ? (parts[1]?.trim() ?? DEFAULT_VALUE)
            : DEFAULT_VALUE;
    } catch (error) {
...
function extractGoyaxMinMax(doc: Document): { min: string; max: string } {
    ...
    try {
        ...
        if (nodes.length === 0) {
            log("SERVICES fetch", {parser: "extractGoyaxMinMax", reason: "missing overview nodes", selector: OVERVIEW_SELECTOR}, "warn");
            return {min: DEFAULT_VALUE, max: DEFAULT_VALUE};
        }
        ...
        if (tables.length < 2) {
            log("SERVICES fetch", {parser: "extractGoyaxMinMax", reason: "missing tables", tables: tables.length}, "warn");
            return {min: DEFAULT_VALUE, max: DEFAULT_VALUE};
        }
        ...
        if (rows.length < 6) {
            log("SERVICES fetch", {parser: "extractGoyaxMinMax", reason: "missing rows", rows: rows.length}, "warn");
            return {min: DEFAULT_VALUE, max: DEFAULT_VALUE};
        }
        ...
        const max = maxCells[3]?.textContent?.trim() ?? DEFAULT_VALUE;
        const min = minCells[3]?.textContent?.trim() ?? DEFAULT_VALUE;
...
function extractAcheckCurrencySymbol(table: Element): string {
    ...
    try {
        ...
        if (rows.length < CURRENCY_ROW + 1) {
            log("SERVICES fetch", {parser: "extractAcheckCurrencySymbol", reason: "missing rows", rows: rows.length}, "warn");
            return DEFAULT_VALUE;
        }
...
function extractAcheckMinMax(table: Element): { min: string; max: string } {
    ...
    try {
        ...
        const max = cells[MAX_CELL]?.textContent?.trim() ?? DEFAULT_VALUE;
        const min = cells[MIN_CELL]?.textContent?.trim() ?? DEFAULT_VALUE;
...
function extractAcheckStockData(doc: Document): FetchResult {
    ...
    if (tables.length < MIN_REQUIRED_TABLES) {
        log("SERVICES fetch", {parser: "extractAcheckStockData", reason: "missing tables", selector: CONTENT_TABLE_SELECTOR, tables: tables.length}, "warn");
        return {
            rate: DEFAULT_VALUE,
            min: DEFAULT_VALUE,
            max: DEFAULT_VALUE,
...
function calculateMidQuote(bid: string, ask: string): string {
    try {
        ...
        if (!Number.isFinite(bidNumber) || !Number.isFinite(askNumber)) {
            log("SERVICES fetch", {parser: "calculateMidQuote", reason: "non-finite bid/ask", bid, ask, bidNumber, askNumber}, "warn");
            return DEFAULT_VALUE;
        }
    ...
    fnet: async (urls: NumberStringPair[]): Promise<StockMarketData[]> => {
        return Promise.all(
            urls.map(async (urlObj: NumberStringPair): Promise<StockMarketData> => {
                const html = await fetchTextWithCacheFollowRedirect(urlObj.value);
    ...
    ard: async (urls: NumberStringPair[]): Promise<StockMarketData[]> => {
        return Promise.all(
            urls.map(async (urlObj: NumberStringPair): Promise<StockMarketData> => {
                ...
                const detailHtml = await fetchWithCache(detailUrl, detailUrl);
 ...
 */
export async function fetchDateData(obj: NumberStringPair[]): Promise<DateData[]> {
...
export async function fetchIndexData(): Promise<StringNumberPair[]> {
    ...
    for (const property of Object.keys(SETTINGS.INDEXES)) {
        for (const link of links) {
            ...
            if (SETTINGS.INDEXES[property]?.includes(title || "") && valueText) {
                indexes.push({
                    key: property,
