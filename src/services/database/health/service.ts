    ...
    async function collectStats(): Promise<HealthStats> {
        ...
        return transactionManager.execute(
            ...
            async (tx: IDBTransaction) => {
                ...
                const accountIds = new Set(accounts.map((a: any) => a.cID));
                ...
                return {
                    ...
                    orphanedBookings: bookings.filter(
                        (b: any) => !accountIds.has(b.cAccountNumberID)
                    ).length,
                    orphanedStocks: stocks.filter(
                        (s: any) => !accountIds.has(s.cAccountNumberID)
                    ).length,
                    orphanedBookingTypes: bookingTypes.filter(
                        (bt: any) => !accountIds.has(bt.cAccountNumberID)
                    ).length
                };
            }
        );
