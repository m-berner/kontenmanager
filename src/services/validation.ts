...
export class ValidationService {
     ...
     */
    static createRule(validator: (_value: unknown) => boolean, message: string): ValidationRuleType {
     ...
     * @returns A validation rule function.
     */
    static oneOfTwo(zeroValue: Ref<number> | number, message: string): ValidationRuleType {
        return ValidationService.createRule(
            v => {
     ...
     * @returns A validation rule function.
     */
    static stringLength(min: number, max: number, message: string): ValidationRuleType {
        return this.createRule(
            (v) => {
    ...
    static amountRules(zeroValue: Ref<number> | number, msgArray: string[]): ValidationRuleType[] {
        return [
    ...
    static ibanRules(msgArray: readonly string[]): ValidationRuleType[] {
        return [
            ...
            this.fromDomain(
                (v) => ValidationRules.validateIBAN(v as string),
                {
    ...
    static isinRules(msgArray: string[]): ValidationRuleType[] {
        return [
            this.required(msgArray[0]),
            this.fromDomain(
                (v) => ValidationRules.validateISIN(v as string),
                {
                    [VALIDATION_CODES.INVALID_LENGTH]: msgArray[1],
                    [VALIDATION_CODES.INVALID_FORMAT]: msgArray[2],
    ...
    static swiftRules(msgArray: readonly string[]): ValidationRuleType[] {
        return [
            this.required(msgArray[0]),
            this.fromDomain(
                (v) => ValidationRules.validateSWIFT(v as string),
                {
                    [VALIDATION_CODES.INVALID_LENGTH]: msgArray[1],
                    [VALIDATION_CODES.INVALID_FORMAT]: msgArray[2],
