...
import {createCompanyHeaders, createCompanyMenuItems, ITEMS_PER_PAGE_OPTIONS} from "@/domains/configs/views";
...
onBeforeMount(async () => {
  ...
  records.stocks.items.forEach((stock) => {
    if (stock.cID > 0) {
      stock.mPortfolio = calculatePortfolioByStockId(
          records.bookings.items,
          stock.cID
      );
      stock.mInvest = calculateInvestByStockId(
          records.bookings.items,
          stock.cID
      );
    }
