...
/**
 * Re-fetches the currently rendered rows when the FX divisors actually
 * change while this view stays mounted.
 *
 * `AppIndex.vue`'s `displayCurrency`/`settings.service` watchers invalidate
 * the freshness markers (`runtime.clearStocksPages()`) only once the new
 * rates have landed — but invalidating a marker doesn't repaint anything
 * already on screen. A quote that finished loading, with the OLD divisor (or
 * the honest-fallback `1`, reset synchronously before the fetch even starts —
 * see `resolveBaseExchangePairs`), in the window between that reset and the
 * new rates arriving stays displayed and marked fresh until something else
 * triggers a fetch. If the user simply stays on this page (no pagination, no
 * re-sort) that never happens: the wrong mValue/mChange/depot figures do not
 * self-correct for the rest of the session.
 *
 * Watching the resolved divisors directly, rather than trying to time this
 * against `AppIndex`'s fetch, is what makes it correct regardless of why they
 * changed (an account-currency switch or a provider change away from
 * "none") and regardless of how that race resolved.
 *
 * Gated the same way `onCurrentItems` gates its own re-fetch: only once this
 * view has already loaded something. Boot's Phase 3 (and any FX refresh
 * before this component ever mounted) writes these divisors too, and must
 * not trigger a fetch here before there is anything rendered to refresh.
 */
watch(() => [runtime.curUsd, runtime.curEur], async () => {
  if (!hasLoadedOnce.value || renderedStockIds.value.length === 0) return;
