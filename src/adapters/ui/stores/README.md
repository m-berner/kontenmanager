...
**Pinia State Stores** manage the application’s reactive state. The Stores Layer serves as a central repository for data
and coordinates between the [Secondary Adapters Layer](/src/adapters/driven/README.md)
for persistence, and the [Components Layer](/src/adapters/ui/components/README.md) for presentation.
...
- **Data Coordination**: Synchronize state across different parts of the application (e.g., updating balances when a
  booking is added).
- **Business Logic Integration**: Utilize functional [Domain Logic](/src/domain/README.md) to provide calculated
...
- **Hub**: `recordsHub.ts` is the primary orchestrator for mass operations (init, clean) and the single import point for
  consumers that need multiple store domains.
...
- **`portfolio.ts`** (`usePortfolioStore`): Combines `stocks`, `bookings`, and `settings` to derive active/passive stock
  lists with computed portfolio values, investment totals, and depot sum (`active`, `passive`, `sumDepot`). Network side
  effects (online price fetching) are handled separately in the `useOnlineStockData` composable.
...
- `deps.ts`: attachStoreDeps, getSettingsStoreDeps, StoreTranslate, attachStoreTranslate,
  getStoreTranslate
