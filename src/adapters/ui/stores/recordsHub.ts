...
export const useRecordsStore = defineStore("records", function () {
    ...
    /**
     * Whether an account is actually selected AND still exists.
     *
     * Distinct from `accounts.items.length > 0`, which is what the header-bar
     * guards used to test. The two were equivalent only by accident: historically
     * `activeAccountId === -1` implied zero accounts. "Accounts exist but none is
     * active" is now a real state, and every action that writes under the active
     * account (add booking / add booking type) or edits it (update / delete
     * account) needs THIS predicate, not a population count.
     *
     * Also rejects a dangling id — one pointing at an account that is no longer
     * in the store — which a bare `> 0` check would wave through.
     */
    const hasActiveAccount = computed((): boolean => {
        const id = settingsStore.activeAccountId;
        return id > 0 && accountsStore.getById(id) !== null;
    });
    ...
    return {
        ...
        isDepot,
        /** Whether an account is selected and still exists. */
        hasActiveAccount,
        init,
        clean
    };
