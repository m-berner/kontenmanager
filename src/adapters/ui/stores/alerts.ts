...
import type {ConfirmationDialogData, VisibleAlertData} from "@/domain/types";
...
/** Resolves the translated default confirm/cancel labels, falling back to English if i18n isn't wired yet. */
function defaultConfirmText(): string {
    const t = getStoreTranslate();
    return t ? t("stores.alerts.confirmText") : "Confirm";
}

function defaultCancelText(): string {
    const t = getStoreTranslate();
    return t ? t("stores.alerts.cancelText") : "Cancel";
}
 ...
 *
 * @returns Unique incrementing ID
 */
...
export const useAlertsStore = defineStore("alerts", () => {
    ...
    /**
     * Resets the confirmation dialog to its default state by assigning a new default confirmation object.
     *
     ...
     * Clears associated timeout and shows the next alert if the dismissed alert was currently displayed.
     *
    ...
    function dismissAlert(id: number | undefined): void {
        ...
        if (index === -1) {
            log("STORES alert: Alert not found for dismissal", id, "warn");
            return;
        }
    ...
    function confirm(
        ...
        return new Promise((resolve, reject) => {
            ...
            if (confirmationDialog.value.id > -1) {
                log(
                    "STORES alert: Confirmation dialog is already active",
                    null,
                    "warn"
                );
                reject(
