...
export const useAccountsStore = defineStore("accounts", function () {
     ...
     *
     * `excludeId` skips the account currently being edited, so re-validating an
     * unchanged IBAN on the update path does not flag the record against itself.
     */
    const isDuplicate = computed(() => (iban: string, excludeId?: number): boolean => {
        return isDuplicateAccountIban(items.value, iban, excludeId);
    });
