...
- `bookingSearch.ts`: Search configuration for `HomeContent`'s bookings table — filter keys plus the
  custom-key-filter that makes the booking-type column searchable by its displayed name, the
  Debit/Credit columns searchable by their locale-formatted text rather than the raw number, and
  the Datum column searchable by its locale-formatted short date (plus the raw ISO year, still).
  All four keys share one function (a Vuetify `filterMode="union"` quirk requires every registered
  `customKeyFilter` to agree, not an independent OR across each — see the file's own doc comment).
  Not a `use*` composable — it holds no reactive state; it lives here because it is a plain module
  rather than a `.vue` screen, and because being importable is what lets it be tested against
  Vuetify's real filter.
- `currencyParsing.ts`: Pure locale-aware parsing (`parseCurrency`, `getSeparators`) behind
  `CurrencyInput.vue`'s text field, extracted for the same reason as `bookingSearch.ts` — a
  `<script setup>` block isn't importable, and this had no test coverage otherwise.
...
- `currencyParsing.ts`: getSeparators, parseCurrency
