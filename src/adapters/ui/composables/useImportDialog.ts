...
export function useImportDatabaseDialogController(input: {
    ...
    const restoreFromRollback = async (rollbackData: RollbackData): Promise<boolean> => {
        ...
        try {
            ...
            //
            // Rows are re-hydrated RAW here, the same as app boot
            // (getAccountRecords -> records.init, deliberately unwrapped —
            // see toRecordsPort's doc comment) — not through
            // validateBooking/applyBookingRoleInvariants, the way the import
            // PLAN builds its initData (buildModernImportPlan) from a raw,
            // not-yet-trusted backup file. rollbackData came from
            // getAllRecords, i.e. IndexedDB itself, so every field it carries
            // already passed those checks whenever it was originally written.
            // Re-sanitizing it here could disagree with the DB write just
            // above (which restores these same rows verbatim): a booking type
            // whose role changed after the row was written — legal, via
            // updateBookingTypeUsecase — would have its cStockID/cCount
            // zeroed in the STORE ONLY, silently diverging from the DB the
            // rest of the rollback just restored, until the next full reload.
            ...
            await input.records.init(
                {
                    ...
                    bookingsDB: rollbackData.bookings.filter((b) => b.cAccountNumberID === activeId),
            ...
            await importDatabaseUsecase(
                ...
                },
                {
                    ...
                    confirmProceed: async (counts, existingCounts) => {
                        ...
                        // Deliberately doesn't also list existingCounts: showing
                        // two count blocks side by side (what's about to be
                        // imported vs. what's about to be deleted) read as if
                        // the import would somehow leave both behind, when only
                        // the first survives. A short note is enough — the
                        // user is about to overwrite their own data, not being
                        // asked to reconcile two datasets.
