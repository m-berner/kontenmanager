...
<template>
  <v-form ref="formRef" validate-on="submit" @submit.prevent>
    ...
    <template v-else>
      ...
      <!--
        Keeps the form permanently invalid while the error boundary is showing.
        The slot is replaced by the alert above, so the v-form has no registered
        fields left and validate() would resolve {valid: true} — submitGuard
        would then run operation() against a form the user was never able to
        fill in. An always-failing hidden input is the least invasive way to
        block that without plumbing a new flag through every dialog and
        DialogPort's OK button.
      -->
      <v-input
          :model-value="null"
          :rules="[() => t('components.dialogs.forms.baseDialogForm.loadError')]"
          class="d-none"
          hide-details/>
    </template>
    <v-overlay
