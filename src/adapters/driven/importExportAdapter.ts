 ...
 *
 * MAX_SIZE deliberately reuses `INDEXED_DB.MAX_FILE_SIZE`, the same limit
 * `useImportDialog.validateFile` enforces in the UI. The two used to disagree
 * (96 MB here vs 64 MB there) — harmless only because the stricter UI check ran
 * first, but two numbers for one rule is exactly how they drift apart. The
 * unused MIN_SIZE constant was removed rather than left as dead configuration.
 */
const FILE_VALIDATION = {
    MAX_SIZE: INDEXED_DB.MAX_FILE_SIZE
} as const;
