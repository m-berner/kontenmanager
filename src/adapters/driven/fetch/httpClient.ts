...
import {getCache, setCache} from "@/adapters/driven/fetch/httpCache";
    ...
    try {
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                // before throwing the timeout error anyway. Stop here instead
                // and let the throw below report it.
...
export async function fetchWithCache(
    ...
    // `!== null` rather than a truthiness test: `getCache` already returns
    // `entry.data` for a cached empty body, and `""` is falsy — so an empty
    // response was read as a miss and re-fetched on *every* call, defeating both
    // the cache and its TTL for that URL while `setCache` rewrote the same empty
    // body each time. `null` is the only value that means "not cached".
...
export async function fetchTextWithCacheFollowRedirect(
    ...
    // `!== null` — see the note in `fetchWithCache`; a cached empty body is a
    // hit, not a miss.
    const cached = getCache(url, ttl);
    if (cached !== null) {
        log(`SERVICES fetch: Cache hit for ${url}`);
