...
function extractArdStockData(doc: Document): FetchResult | null {
    ...
    const rawCell = (row: HTMLTableRowElement, cellIndex: number): string =>
        row.cells[cellIndex]?.textContent ?? DEFAULT_VALUE;

    const stripCurrency = (value: string): string =>
        value.replace(DEFAULT_CURRENCY_SYMBOL, "");

    const cleanCell = (row: HTMLTableRowElement, cellIndex: number): string =>
        stripCurrency(rawCell(row, cellIndex));

    // Prefer label-based extraction since row order is prone to change.
    // Returns the RAW cell text so the caller can read the currency marker off
    // it before it is stripped.
    //
    // Matches the label as a whole (after trimming a trailing ":"), not as a
    // substring: `label.includes("kurs")` also matched "Kursziel" (price
    // target) and "Vortageskurs" (previous close), and the first such row in
    // document order won — the same failure mode round 30 found on acheck,
    // where previous close was silently returned as the current price.
    const matchesLabel = (label: string, terms: string[]): boolean => {
        const normalized = label.replace(/:$/, "").trim();
        return terms.some((term) => normalized === term.toLowerCase());
    };

    const findRawValueByLabel = (terms: string[]): string | null => {
        for (const row of rows) {
            const label = row.cells[0]?.textContent?.trim().toLowerCase() ?? "";
            if (!label) continue;
            if (matchesLabel(label, terms)) {
                return rawCell(row, 1);
            }
        }
        return null;
    };
    ...
    // by the USD rate.
    const currencyOf = (raw: string | null): string =>
        (raw ? detectCurrency(raw) : "") || DEFAULT_CURRENCY;

    const rawRateByLabel = findRawValueByLabel(["Kurs"]);
    const rawMinByLabel = findRawValueByLabel(["52 Wochen-Tief"]);
    const rawMaxByLabel = findRawValueByLabel(["52 Wochen-Hoch"]);

    if (rawRateByLabel) {
        return {
            rate: stripCurrency(rawRateByLabel),
            min: rawMinByLabel !== null ? stripCurrency(rawMinByLabel) : DEFAULT_VALUE,
            max: rawMaxByLabel !== null ? stripCurrency(rawMaxByLabel) : DEFAULT_VALUE,
            currency: currencyOf(rawRateByLabel)
        };
    }
    ...
    if (rows.length >= 8) {
        return {
            ...
            currency: currencyOf(rawCell(rows[0], 1))
        };
    }
