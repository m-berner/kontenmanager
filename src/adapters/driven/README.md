...
This directory contains the **infrastructure adapters** of the application. The secondary adapters layer is responsible
for interacting with external systems (IndexedDB, WebExtension APIs, external web APIs) and coordinating the flow of
data between the [Domain Layer](/src/domain/README.md) and the application/UI layers.
...
- **Shared helpers** (`fetch/providerUtils.ts`): Constants (`DEFAULT_VALUE`, `DEFAULT_CURRENCY`,
  `DEFAULT_CURRENCY_SYMBOL`) and pure helpers (`detectCurrency`, `parseCurrency`, `calculateMidQuote`) used across
  providers.
...
`importExportAdapter` is created via `createImportExportAdapter()` and provided via the DI container (it is not a global
singleton).
...
- `importExportAdapter.ts`: validateBackupData, validateDataIntegrityStatus, stringifyDatabase,
