...
export function createDatabaseAdapter(
    ...
    /**
     * Retrieves the complete contents of all four stores, for every account.
     *
     * Deliberately distinct from `getAccountRecords`, which is asymmetric by
     * design: it returns ALL accounts but only the *active* account's
     * bookings/bookingTypes/stocks, because that is what the in-memory stores
     * hold. Anything that needs a faithful whole-database snapshot — notably
     * the import rollback point, which restores with a global `clear` — must
     * use this instead, or it will re-add only the active account's rows after
     * wiping every account's.
     *
     * Read in a single readonly transaction so the four stores cannot be
     * observed at different points in time.
     */
    async function getAllRecords(): Promise<RecordsDbData> {
        log("DATABASE service: fetching all records");
