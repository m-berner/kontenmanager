...
export function createTransactionManager(connection: DatabaseConnection) {
    ...
    async function execute<T>(
        ...
        // Attach the completion handlers BEFORE running the operation. An
        // IndexedDB transaction auto-commits as soon as control returns to the
        // event loop with no pending requests, so if `operation` ever awaits a
        // non-IDB promise (a timer, a fetch) the transaction commits and fires
        // `oncomplete` first — and handlers attached afterwards would never run,
        // leaving the returned promise pending forever.
        const completion = waitForCompletion(tx, stores, mode);
        // The rejection is consumed below; this only stops it being reported as
        // unhandled in the window before `operation` finishes.
