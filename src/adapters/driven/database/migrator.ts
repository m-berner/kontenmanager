 ...
 * (domain/logic.ts) only ever resolves the first match, so a second same-role type's
 * bookings would be silently excluded from that role's portfolio/invest/dividend totals
 * forever, with no way to fix it afterward: `updateBookingTypeUsecase` refuses a rename
 * that would create exactly this collision, and a role-protected type cannot be deleted
 * (`deleteBookingTypeUsecase`) to re-add it under "other". The import path already guards
 * against this same class of legacy-inference collision
 * (`domain/importExport/validator.ts`'s `checkDuplicateBookingTypeRoles`, which explicitly
...
function backfillBookingTypeRoles(tx: IDBTransaction): void {
    ...
    const store = tx.objectStore(storeName);
    // Every row's role claims a "slot" for its account, whether this backfill
    // infers it or the row already carried an explicit one — a later
    // NEWLY-INFERRED row must not collide with either kind. Rows that already
    // have a valid, explicit `cRole` are never demoted here: this backfill's
    // job is to stop itself from manufacturing a new collision, not to
    // re-litigate roles the app's own usecases already guarded when they were
    // set.
    const rolesTakenByAccount = new Map<number, Set<string>>();
    ...
    request.onsuccess = (event) => {
        ...
        const takenRoles = rolesTakenByAccount.get(record.cAccountNumberID) ?? new Set<string>();
        rolesTakenByAccount.set(record.cAccountNumberID, takenRoles);

        if (!record.cRole) {
            let inferredRole = resolveLegacyBookingTypeRole(record.cID, record.cName);
            if (inferredRole !== BOOKING_TYPE_ROLE.OTHER && takenRoles.has(inferredRole)) {
            ...
            if (inferredRole !== BOOKING_TYPE_ROLE.OTHER) takenRoles.add(inferredRole);
        } else if (record.cRole !== BOOKING_TYPE_ROLE.OTHER) {
            takenRoles.add(record.cRole);
        }
        cursor.continue();
    };
