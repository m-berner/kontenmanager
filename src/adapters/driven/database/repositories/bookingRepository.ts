...
export function createBookingRepository(transactionManager: TransactionManagerContract) {
    ...
    async function countByAccount(accountId: number): Promise<number> {
        // Counts through the index instead of materializing every row via
        // `findByAccount(...).length` — see `baseRepository.countBy`. This is
        // the repository where it matters most: an account's booking history is
