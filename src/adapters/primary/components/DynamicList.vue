...
onBeforeMount(async () => {
  ...
  try {
    ...
    switch (props.type) {
      case COMPONENTS.DYNAMIC_LIST.TYPES.EXCHANGES: {
        const exchanges = storage[BROWSER_STORAGE.EXCHANGES.key];
        list.value = Array.isArray(exchanges)
            ? exchanges.filter(
        ...
        break;
      case COMPONENTS.DYNAMIC_LIST.TYPES.MARKETS: {
        const markets = storage[BROWSER_STORAGE.MARKETS.key];
        list.value = Array.isArray(markets)
            ? markets.filter(
                (entry): entry is string => typeof entry === "string"
            )
            : [];
