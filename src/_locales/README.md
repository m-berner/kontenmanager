...
3. **Placeholders**: Use standard WebExtension placeholders for dynamic content (e.g., `$1`, `$2`) in `messages.json`. For `gui.json`, use `vue-i18n` style placeholders (e.g., `{name}`).
