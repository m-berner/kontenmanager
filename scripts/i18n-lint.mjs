...
const IGNORE_KEYS = new Set([
  // example: 'components.dialogs.forms.accountForm.messages.onUpdateIsin'
]);
...
function extractUsedKeysFromText(text) {
  ...
  // $t('key') in Options API / templates
  const re$t = /\$t\(\s*(["'`])([^"'`)]+)\1\s*\)/g;
  // <i18n-t keypath="key"> in templates
  const reI18nT = /<i18n-t[^>]*\skeypath=(["'])([^"']+)\1/gi;

  for (const re of [reT, re$t, reI18nT]) {
    let m;
