...
test("background smoke (firefox): registers listeners and initializes storage defaults", async ({
                                                                                                    page,
                                                                                                    browserName
                                                                                                }) => {
    ...
    try {
        ...
        await page.waitForFunction(() => {
            const t = (window as unknown as {
                __kmTest?: { installedListeners: unknown[]; clickedListeners: unknown[] }
            }).__kmTest;
        ...
        const storageSetCalls = await page.evaluate(() => {
            const t = (window as unknown as {
                __kmTest: { calls: { storageSet: Array<Record<string, unknown>> } }
            }).__kmTest;
        ...
        await page.evaluate(() => {
            const t = (window as unknown as {
                __kmTest: { state: { tabsQueryResult: Array<{ id?: number; windowId?: number }> } }
            }).__kmTest;
            t.state.tabsQueryResult = [
                {id: 10, windowId: 1},
                {id: 11, windowId: 1}
