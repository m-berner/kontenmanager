...
test("happy path (firefox): import backup and see Company content", async ({page, browserName}) => {
  ...
  try {
    ...
    // Keep app boot off the real internet — see blockExternalRequests doc comment.
    await blockExternalRequests(page, server.baseUrl);
    ...
    // 45s matches bootWithFixtureImported's wait for this same locator: these
    // tests run in the shared parallel suite, so app boot competes for CPU with
    // every other worker's Firefox instance.
    const headerBar = page.locator("header.v-app-bar").first();
    await expect(headerBar).toBeVisible({timeout: 45_000});
    ...
    // resolving `.last()` — see its doc comment for the race this avoids.
...
test("add company by ISIN (firefox): create new company with ISIN DE000BASF111", async ({page, browserName}) => {
  ...
  try {
    ...
    // Wait for header bar (45s — see the first test's note on parallel CPU contention)
    const headerBar = page.locator("header.v-app-bar").first();
    await expect(headerBar).toBeVisible({timeout: 45_000});
