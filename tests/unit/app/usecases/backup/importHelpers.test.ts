...
describe("usecases/backup/importHelpers", () => {
    describe("getImportCounts", () => {
        ...
        it("does not count a numeric (UNIX timestamp) cBookDate as undated", () => {
            // normalizeDate (validators.ts) accepts a finite number as a UNIX
            // timestamp and converts it to a real ISO date via isoDate — it does
            // not blank it out. A backup with a raw JS Date().getTime() value in
            // cBookDate (nothing enforces the field is a string before this
            // point) must not trigger the "N bookings have no date" prompt for
            // rows that will, in fact, import with a correct date.
            const backup: ModernBackupData = {
                sm: {cVersion: "1", cDBVersion: INDEXED_DB.CURRENT_VERSION, cEngine: "x"},
                accounts: [],
                stocks: [],
                bookings: [
                    makeBookingDb({cID: 1, cBookDate: 1700000000000 as unknown as string}),
                    // A number outside Date's representable range is still
                    // genuinely undated, since isoDate itself would reject it.
                    makeBookingDb({cID: 2, cBookDate: 1e20 as unknown as string})
                ],
                bookingTypes: []
            };

            expect(getImportCounts(backup).undatedBookings).toBe(1);
        });
    });
    ...
    describe("buildModernImportPlan", () => {
        ...
        it("scopes initData to the active account id, excluding other accounts' records", () => {
            const backup: ModernBackupData = {
                ...
                bookingTypes: [makeBookingTypeDb({cAccountNumberID: 1}), makeBookingTypeDb({
                    cID: 2,
                    cAccountNumberID: 2
                })]
            };
