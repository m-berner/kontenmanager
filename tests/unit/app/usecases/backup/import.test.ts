...
function makeDeps(input: {
    ...
    const records = createRecordsPortMock({
        accountIds: input.existingAccountIds,
        stockIds: input.existingStockIds
    });
...
describe("usecases/backup/import", () => {
    ...
    it("does not count the cID-0 placeholder stock as existing data", async () => {
        // Regression: `records.init` (domain/logic.ts's initializeRecords)
        // unconditionally prepends a cID-0 placeholder stock to the stocks
        // store, including on a fresh install with zero accounts — so
        // `existingCounts.stocks` was always >= 1, and the import
        // confirmation warned about deleting data on a genuinely empty
        // database.
        const backup = {
            sm: {cVersion: "1", cDBVersion: MODERN, cEngine: "x"},
            accounts: [makeAccountDb({cID: 1})],
        ...
        expect(input.confirmProceed).toHaveBeenCalledWith(
            expect.anything(),
            expect.objectContaining({accounts: 0, stocks: 0})
        );
    });
