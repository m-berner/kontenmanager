...
describe("useForms", () => {
    describe("createStockFormManager", () => {
        it("starts with the documented default field values", () => {
            ...
            // `boolean`, matching the v-checkbox that writes it, rather than the
            // `1 | 0` the DB column uses. The literal union was the most precise
            // declaration in StockFormData and the only one nothing upheld:
            // Vuetify's v-model prop is typed `any`, so a click assigned
            // true/false unchecked, and UpdateStock's Object.assign copied a
            // widened `number` off StockDb. `mapStockFormToDb` converts with
            // `data.fadeOut ? 1 : 0`, so the mapper is where 1/0 is produced.
            expect(stockFormData.fadeOut).toBe(false);
            expect(stockFormData.firstPage).toBe(false);
        });
    ...
    describe("createBookingTypeFormManager", () => {
        it("starts with a null id (distinguishes add from update) and role 'other'", () => {
            const {bookingTypeFormData} = createBookingTypeFormManager();
            expect(bookingTypeFormData.id).toBeNull();
            expect(bookingTypeFormData.role).toBe("other");
        });
