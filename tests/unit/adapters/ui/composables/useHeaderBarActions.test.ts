...
describe("useHeaderBarActions", () => {
    ...
    describe("account-gated actions", () => {
        ...
        it("updateAccount opens the dialog once an account is active", async () => {
        ...
        // "Accounts exist" and "an account is active" came apart once a storage
        // reset could leave the selection at the sentinel with the accounts still
        // loaded. These actions all operate ON the active account, so the
        // population count was the wrong precondition: UpdateAccount rendered a
        // blank form and DeleteAccountConfirmation reported success for deleting
        // nothing.
        it.each(["updateAccount", "deleteAccountConfirmation", "addBookingType", "addBooking"])(
            "%s stays closed when accounts exist but none is active",
            async (action) => {
                const records = useRecordsStore();
                const runtime = useRuntimeStore();
                const settings = useSettingsStore();
        ...
        it("rejects a dangling active id pointing at an account that no longer exists", async () => {
            const records = useRecordsStore();
            const runtime = useRuntimeStore();
            const settings = useSettingsStore();
            records.accounts.add(makeAccountDb({cID: 1}));
        ...
        it("exportDatabase stays gated on ANY account, not on an active one — it exports the whole database", async () => {
            const records = useRecordsStore();
            const runtime = useRuntimeStore();
            const settings = useSettingsStore();
            records.accounts.add(makeAccountDb({cID: 1}));
            ...
            expect(runtime.dialogName).toBe("exportDatabase");
        });

        it("exportDatabase is gated on having at least one account", async () => {
            const runtime = useRuntimeStore();
            const {onIconClick} = useHeaderBarActions(t);
