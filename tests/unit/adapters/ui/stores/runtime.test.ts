...
describe("Runtime Store", () => {
    ...
    it("invalidateStocksPage should bump the page's generation so an in-flight fetch's stale write is discarded", () => {
        const runtime = useRuntimeStore();

        const staleGeneration = runtime.bumpStocksPageGeneration(1);
        runtime.markStocksPageLoaded(1);

        runtime.invalidateStocksPage(1);
    ...
    it("clearStocksPages should bump the generation of every tracked page, not just clear the loaded set", () => {
        ...
        const staleGen2 = runtime.bumpStocksPageGeneration(2);
        runtime.markStocksPageLoaded(1);
        runtime.markStocksPageLoaded(2);

        runtime.clearStocksPages();

        expect(runtime.loadedStocksPages.size).toBe(0);
        expect(runtime.isStocksPageGenerationCurrent(1, staleGen1)).toBe(false);
