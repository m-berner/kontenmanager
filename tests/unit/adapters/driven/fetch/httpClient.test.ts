...
describe("httpClient", () => {
    ...
    describe("fetchWithCache", () => {
        it("fetches and caches on a miss", async () => {
            fetchMock.mockResolvedValue({
                ok: true,
                status: 200,
                text: vi.fn().mockResolvedValue("<html lang=''>fresh</html>")
            } as unknown as Response);

            const result = await fetchWithCache("https://example.com/page");

            expect(result).toBe("<html lang=''>fresh</html>");
            expect(fetchMock).toHaveBeenCalledTimes(1);
        });
    ...
    describe("fetchTextWithCacheFollowRedirect", () => {
        it("caches the response under both the original and the redirected URL", async () => {
            fetchMock.mockResolvedValue({
                ...
                text: vi.fn().mockResolvedValue("<html lang=''>redirected</html>")
            } as unknown as Response);

            const first = await fetchTextWithCacheFollowRedirect("https://example.com/original");
            expect(first).toBe("<html lang=''>redirected</html>");
            expect(fetchMock).toHaveBeenCalledTimes(1);
            ...
            expect(second).toBe("<html lang=''>redirected</html>");
            expect(third).toBe("<html lang=''>redirected</html>");
            expect(fetchMock).toHaveBeenCalledTimes(1);
        });
    });
    ...
    describe("parseHTML", () => {
        ...
        it("parses HTML text into a Document", async () => {
            const doc = await parseHTML("<html lang=''><body><p>hi</p></body></html>");
