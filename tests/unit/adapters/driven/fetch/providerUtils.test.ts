...
describe("adapters/driven/fetch/providerUtils", () => {
    describe("calculateMidQuote", () => {
        ...
        it("forces German decimal parsing when a locale is given, overriding auto-detection", () => {
            // "1,3105" has 5 characters after the comma, so unforced auto-detect
            // (detectNumberFormat) reads it as English thousands-grouping and
            // strips the comma instead of treating it as a 4-decimal fraction.
            // A caller whose source is always comma-decimal (tgateFetcher) must
            // be able to force the correct reading.
            expect(calculateMidQuote("1,3105", "1,3170", "de")).toBe("1.31375");
            expect(calculateMidQuote("1,3105", "1,3170")).not.toBe("1.31375");
        });
    });
