...
describe("migrator: setupDatabase", () => {
    it("creates all four object stores with their indexes on a fresh database", () => {
        ...
        const ev = {
            oldVersion: 0,
            newVersion: 27,
            target: {transaction: createFakeUpgradeTx()}
    ...
    describe("backfillBookingTypeRoles (oldVersion < 28)", () => {
        ...
        // Regression: a buy/sell/dividend role must stay unique per account —
        // resolveTypeIdByRole only ever resolves the first match, so a second
        // same-role type's bookings would be silently excluded from that
        // role's totals forever, with no dialog able to fix it afterward.
        // openCursor() iterates the store's primary key (cID) ascending, so
        // these fixtures are deliberately ordered lowest-cID-first, matching
        // real IndexedDB iteration.
        it("demotes every same-role collision this backfill would create, keeping only the lowest cID's role", () => {
            const records: BookingTypeDb[] = [
                {cID: 3, cName: "Dividende", cAccountNumberID: 1} as unknown as BookingTypeDb,
                {cID: 150, cName: "Dividende", cAccountNumberID: 1} as unknown as BookingTypeDb
            ];

            run(records);

            expect(records[0].cRole).toBe(BOOKING_TYPE_ROLE.DIVIDEND);
            expect(records[1].cRole).toBe(BOOKING_TYPE_ROLE.OTHER);
        });

        it("does not treat the same role in two different accounts as a collision", () => {
            const records: BookingTypeDb[] = [
                {cID: 3, cName: "Dividende", cAccountNumberID: 1} as unknown as BookingTypeDb,
                {cID: 4, cName: "Dividende", cAccountNumberID: 2} as unknown as BookingTypeDb
            ];

            run(records);

            expect(records[0].cRole).toBe(BOOKING_TYPE_ROLE.DIVIDEND);
            expect(records[1].cRole).toBe(BOOKING_TYPE_ROLE.DIVIDEND);
        });

        it("treats an already-explicit role as reserving its account's slot too, against a later newly-inferred row", () => {
            const records: BookingTypeDb[] = [
                {cID: 3, cName: "Dividende", cAccountNumberID: 1, cRole: BOOKING_TYPE_ROLE.DIVIDEND},
                {cID: 150, cName: "Dividende", cAccountNumberID: 1} as unknown as BookingTypeDb
            ];
