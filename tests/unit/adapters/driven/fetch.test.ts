...
import {BROWSER_STORAGE, CACHE_POLICY} from "@/domain/constants";
...
describe("FetchService", () => {
    ...
    describe("fetchDateData", () => {
        ...
        it("does not retry a failed lookup on every call within the backoff window", async () => {
            // Regression: a failure was previously not remembered at all, so
            // under the known finanzen.net 403, every stock whose meeting/
            // quarter day is in the past was re-requested on every page
            // load/reload — one request per affected stock, indefinitely.
            // A real 403 Response, not a rejected promise: fetchWithRetry
            // treats a 403 as final (only 429/5xx are retried), so this is
            // exactly one fetch() call per fetchDateData call, matching the
            // real Akamai-403 scenario.
            const fetchMock = vi
                .spyOn(globalThis, "fetch")
                .mockResolvedValue(new Response("forbidden", {status: 403}));

            const first = await fetchAdapter.fetchDateData([{key: 1, value: "Example"}]);
            expect(first).toEqual([]);
            expect(fetchMock).toHaveBeenCalledTimes(1);

            const second = await fetchAdapter.fetchDateData([{key: 1, value: "Example"}]);
            expect(second).toEqual([]);
            // No new network request: the failure from the first call is
            // still within CACHE_POLICY.DATE_LOOKUP_FAILURE_TTL_MS.
            expect(fetchMock).toHaveBeenCalledTimes(1);
        });

        // Regression: the backoff's catch could not tell a cancellation from a
        // host failure, so it armed the 10-minute marker for both. Aborts here
        // are ordinary, not exceptional — CompanyContent's startOnlineLoad()
        // aborts the previous controller on every onCurrentItems emission (a
        // page change, a re-sort, any change to the rendered row set) and on
        // the FX-divisor watcher — so one pagination click suppressed
        // meeting/quarter-date lookups for every in-flight stock for the next
        // ten minutes.
        it("does not arm the backoff when the caller cancels the lookup", async () => {
            const controller = new AbortController();
            const fetchMock = vi
                .spyOn(globalThis, "fetch")
                .mockImplementation(() => {
                    // Mirrors a real in-flight cancellation: the caller aborts,
            ...
            const aborted = await fetchAdapter.fetchDateData(
                [{key: 1, value: "Example"}],
                {signal: controller.signal}
            );
            // Still omitted, so the caller leaves the stock's stored dates
            // alone — the abort only changes whether the failure is REMEMBERED.
            expect(aborted).toEqual([]);
            expect(fetchMock).toHaveBeenCalledTimes(1);

            // A later, uncancelled call must reach the network again. If the
            // abort had been cached, this would short-circuit and the call
            // count would stay at 1.
            fetchMock.mockResolvedValue(new Response("forbidden", {status: 403}));
            const retried = await fetchAdapter.fetchDateData([{key: 1, value: "Example"}]);
            expect(retried).toEqual([]);
            expect(fetchMock).toHaveBeenCalledTimes(2);
        });

        it("retries a failed lookup again once the backoff window has passed", async () => {
            vi.useFakeTimers();
            try {
                const fetchMock = vi
                    .spyOn(globalThis, "fetch")
                ...
                await fetchAdapter.fetchDateData([{key: 1, value: "Example"}]);
                expect(fetchMock).toHaveBeenCalledTimes(1);

                await vi.advanceTimersByTimeAsync(CACHE_POLICY.DATE_LOOKUP_FAILURE_TTL_MS + 1);
