...
describe("DomainLogic", () => {
    ...
    describe("resolveAccountingTotal", () => {
        it("adds taxes and fees into the total and shows the rows when either is non-zero", () => {
            expect(DomainLogic.resolveAccountingTotal(1000, 100, 10)).toEqual({
                showTaxesAndFees: true,
                total: 1110
            });
            expect(DomainLogic.resolveAccountingTotal(1000, 0, 10)).toEqual({
                showTaxesAndFees: true,
                total: 1010
            });
            expect(DomainLogic.resolveAccountingTotal(1000, 100, 0)).toEqual({
                showTaxesAndFees: true,
                total: 1100
            });
        });

        it("hides the rows and returns finalSum alone when both are zero", () => {
            expect(DomainLogic.resolveAccountingTotal(1000, 0, 0)).toEqual({
                showTaxesAndFees: false,
                total: 1000
            });
        });

        // Regression: ShowAccounting used to gate this on `records.isDepot`
        // instead of on the figures themselves. `cWithDepot` can be switched
        // off on an account that still carries Buy/Sell/Dividend booking
        // types and fee/tax-bearing bookings (turning it off deletes
        // nothing), so a formerly-depot account could still have real,
        // non-zero fees/taxes that the old gate silently dropped from both
        // the visible rows and the total — disagreeing with
        // calculateTotalSum, which always subtracts fees+taxes regardless of
        // the account's current depot flag.
        it("still shows and counts non-zero fees/taxes for a formerly-depot account", () => {
            // Simulates a formerly-depot account (isDepot now false) that
            // still has a booked fee, matching calculateTotalSum's own
            // unconditional treatment of the same booking.
            expect(DomainLogic.resolveAccountingTotal(-1000, 0, 10)).toEqual({
                showTaxesAndFees: true,
                total: -990
            });
