...
- `dialog-actions.spec.ts`: HeaderBar dialog actions against an imported fixture — add/update/delete account,
  ...
  bookings, update/delete stock, show dividend (empty state), export database, show accounting, fade-in-stock (empty
  state), manual quote refresh, the depot-sum chip, the `Ctrl+Alt+R` storage-reset shortcut, Help/Privacy
...
- `npm run test:e2e:headed` runs real, visible Firefox windows, which fight each other for OS
  ...
  still occasionally lack real OS focus, e.g. if the terminal/IDE holds it instead). Headless runs (`test:e2e`, CI) stay
  at `retries: 0` with full parallelism.
