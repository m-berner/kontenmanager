...
4. **Persistence:** Data is persisted via **Services** and **Repositories** (`src/services/database/`) to **IndexedDB** or **Browser Storage** (`storageAdapter`).
5. **Browser Integration:** Interactions with the WebExtension API are abstracted through dedicated **Services** (`src/services/browserService`).
