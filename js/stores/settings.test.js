...
describe("Settings Store", () => {
    ...
    it("setters should update state and persist via setStorage", async () => {
        ...
        expect(mockSetStorage).toHaveBeenCalledWith(BROWSER_STORAGE.BOOKINGS_PER_PAGE.key, 12);
        await store.setStocksPerPage(18);
        expect(store.stocksPerPage).toBe(18);
        expect(mockSetStorage).toHaveBeenCalledWith(BROWSER_STORAGE.STOCKS_PER_PAGE.key, 18);
        await store.setDividendsPerPage(22);
        expect(store.dividendsPerPage).toBe(22);
        expect(mockSetStorage).toHaveBeenCalledWith(BROWSER_STORAGE.DIVIDENDS_PER_PAGE.key, 22);
        await store.setSumsPerPage(7);
        expect(store.sumsPerPage).toBe(7);
        expect(mockSetStorage).toHaveBeenCalledWith(BROWSER_STORAGE.SUMS_PER_PAGE.key, 7);
        await store.setActiveAccountId(5);
        expect(store.activeAccountId).toBe(5);
        expect(mockSetStorage).toHaveBeenCalledWith(BROWSER_STORAGE.ACTIVE_ACCOUNT_ID.key, 5);
    });
});
