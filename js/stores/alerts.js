...
export const useAlertStore = defineStore("alerts", () => {
    ...
    function dismissAlert(id) {
        if (id === undefined) {
            DomainUtils.log("STORES alert: Attempted to dismiss alert with undefined ID", null, "warn");
            return;
        }
    ...
    function confirm(title, message, options) {
        return new Promise((resolve, reject) => {
            if (confirmationDialog.value.id > -1) {
                DomainUtils.log("STORES alert: Confirmation dialog already active", null, "warn");
    ...
    function clearAll() {
        ...
        if (confirmationDialog.value.id > -1 && confirmationDialog.value.reject) {
            try {
                ...
                DomainUtils.log("STORES alert: Error clearing confirmation dialog", err, "warn");
            }
        }
        confirmationDialog.value = { ...defaultConfirmation };
    }
