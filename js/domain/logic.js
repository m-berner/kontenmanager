...
import { DEFAULTS } from '@/domain/config/defaults';
import { DATE } from '@/domain/config/date';
import { SESSION_STORAGE } from '@/composables/storage';
...
export class DomainLogic {
    ...
    static async initializeRecords(storesDB, stores, messages, removeAccounts = true) {
        ...
        const session = await browser.getStorage([SESSION_STORAGE.HIDE_IMPORT_ALERT.key]);
        if (stores.accounts.items.length === 0 && !session[SESSION_STORAGE.HIDE_IMPORT_ALERT.key]) {
            ...
            await browser.setStorage(SESSION_STORAGE.HIDE_IMPORT_ALERT.key, true);
        }
    }
}
