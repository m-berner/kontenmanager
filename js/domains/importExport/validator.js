...
export class ImportExportValidator {
        ...
        if (isLegacy) {
            if (!backup.stocks || !Array.isArray(backup.stocks)) {
                return {
                    isValid: false,
                    version: backup.sm.cDBVersion,
                    error: "Missing or invalid stocks data"
                };
            }
            if (!backup.transfers || !Array.isArray(backup.transfers)) {
                return {
                    isValid: false,
                    version: backup.sm.cDBVersion,
                    error: "Missing or invalid transfers data"
                };
            }
        }
        else {
    ...
    static validateLegacyDataIntegrity(backup) {
        ...
        const duplicateStocks = this.findDuplicates(backup.stocks
            .map((s) => s.cID)
    ...
    static checkDuplicateIds(backup) {
        const errors = [];
        const duplicateAccounts = this.findDuplicates(backup.accounts
            .map((a) => a.cID)
            .filter((id) => id !== undefined));
        ...
        if (duplicateStocks.length > 0)
            errors.push(`Duplicate stock IDs: ${duplicateStocks.join(", ")}`);
        const duplicateBookings = this.findDuplicates(backup.bookings
            .map((b) => b.cID)
            .filter((id) => id !== undefined));
