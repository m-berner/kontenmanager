...
### Project-Specific Notes (kontenmanager)

- Doc set: root `README.md`, one `README.md` per hexagonal-layer folder under `src/`
  (`domain/`, `app/usecases/`, `adapters/ui/*`, `adapters/driven/*`, …), `tests/README.md`,
  `scripts/README.md`.
- There is no standalone `src/WORKFLOWS.md` anymore — its full content (all 13 workflow sections)
  was merged into `src/app/usecases/README.md`'s **`## Workflows`** section (2026-08-13), since
  almost every step it documents funnels through that layer. When this skill's generic playbook
  says to update `WORKFLOW.md`/`WORKFLOWS.md`, for this project that means editing the `## Workflows`
  section of `src/app/usecases/README.md` instead — don't recreate a separate top-level file.
- There is likewise no standalone `src/ARCHITECTURE.md` anymore — its full content (all 17
  sections) was merged into `src/README.md`'s **`## Architecture`** section (2026-08-13), for the
  same reason (one fewer doc to keep in sync). When this skill's generic playbook says to update
  `ARCHITECTURE.md`, for this project that means editing the `## Architecture` section of
  `src/README.md` instead.
- Nested module READMEs end with a mechanically-styled **`## Directory Structure`** appendix
  (`### Directories` / `### Files`, each file listed with its exported symbols). This was
  originally generated by `scripts/update-readmes.mjs` and an `npm run update:readmes` script —
  **both were deleted** (commit `3f50173`, 2026-07-03) in favor of this skill's manual playbook.
  There is currently no automated generator; keep these appendices in sync by hand (or delegate
  to a subagent) when a folder's exported functions/files change — don't reintroduce references
  to the old script.
- Root `README.md` has historically drifted after refactors: it once described a pre-hexagonal
  layout (`src/composables/`, `src/domains/`, `src/services/`, `src/stores/`, …) side-by-side with
  correct hexagonal-layout sections lower in the same file. Always diff the root README's
  "Project Structure" prose against the actual `src/adapters|app|domain` tree, not just against
  `src/README.md`'s "Architecture" section — the two can silently disagree.
...
2. Review current structure and decide deltas
   ...
   - Specifically hunt for **dangling references**: any `npm run <script>` mentioned in docs
     whose entry was removed from `package.json`, or whose target file no longer exists on disk
     (this happens whenever a script/file is deleted but its mentions across docs aren't grepped
     for and cleaned up in the same change). Same check for relative markdown links to nested
     `README.md`/`ARCHITECTURE.md` files that were moved or never existed.
...
Find npm scripts referenced in docs whose target file is missing (dangling reference check):
```
# Extract npm script names actually defined in package.json
$defined = (Get-Content -Raw package.json | ConvertFrom-Json).scripts.PSObject.Properties.Name

# Find "npm run X" mentions in docs that aren't in package.json
Get-ChildItem -Recurse -Include README.md,ARCHITECTURE.md,WORKFLOWS.md,WORKFLOW.md -File | ForEach-Object {
  $matches = Select-String -Path $_.FullName -Pattern 'npm run ([\w:-]+)' -AllMatches
  foreach ($m in $matches.Matches) {
    $name = $m.Groups[1].Value
    if ($defined -notcontains $name) {
      Write-Host "[DANGLING] $($_.FullName): npm run $name (not in package.json)"
    }
  }
}
```

Find relative markdown links pointing at files that don't exist:
```
Get-ChildItem -Recurse -Filter README.md | ForEach-Object {
  $dir = $_.DirectoryName
  Select-String -Path $_.FullName -Pattern '\]\((?!https?://)([^)]+\.md)\)' -AllMatches |
    ForEach-Object { $_.Matches } | ForEach-Object {
      $target = Join-Path $dir $_.Groups[1].Value
      if (-not (Test-Path $target)) { Write-Host "[BROKEN LINK] $($_.Groups[1].Value) in $dir" }
    }
}
```
