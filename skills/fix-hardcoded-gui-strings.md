...
foreach ($f in $files) {
    ...
    for ($i = 0; $i -lt $lines.Count; $i++) {
        ...
        # HTML comments (`<!-- ... -->`) inside `<template>` are near-universal in this
        # codebase's Vue files — every non-trivial component carries multi-line design-
        # rationale comments in the template, the same as `.ts` files do with `//`. Without
        # this check, every prose line inside one is indistinguishable from real rendered
        # text to the regexes below, and dominates the findings (confirmed: a real run
        # surfaced 137 hits, of which 136 were comment prose and 0 were genuine hardcoded
        # strings). Handles the multi-line `<!--` / `-->` on their own lines case this
        # codebase actually uses; a `<!-- inline -->` sharing a line with real markup is
        # not stripped mid-line and can still produce a stray hit — eyeball those.
        if ($f.Extension -ieq ".vue" -and $inTemplate) {
            if (-not $inHtmlComment -and $line -match "<!--" -and $line -notmatch "-->") {
                $inHtmlComment = $true
                continue
            }
            if ($inHtmlComment) {
